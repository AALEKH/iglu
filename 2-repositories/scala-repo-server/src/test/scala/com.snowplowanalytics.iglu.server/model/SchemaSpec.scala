...
class SchemaSpec extends Specification with SetupAndDestroy {
  ...
  val vendorPrefix = "com.snowplowanalytics"
  val otherVendorPrefix = "com.unit"
  val faultyVendorPrefix = "com.benfradet"
  ...
  "SchemaDAO" should {
    ...
    "for update" should {
      ...
      "update a schema properly" in {
        val (status, res) = schema.update(vendor, name, format, version,
          invalidSchema2, vendorPrefix, permission)
        status === OK
      ...
      "create a schema if it does not exist" in {
        val (status, res) = schema.update(vendor, name3, format, version,
          invalidSchema, vendorPrefix, permission)
        status === Created
        res must contain("Schema successfully added") and contain(vendor)
    ...
    "for getMetadata" should {
      ...
      "retrieve metadata about a schema properly if it is public" in {
        val (status, res) = schema.getMetadata(otherVendors, name2s,
          formats, versions, faultyVendorPrefix, permission)
        status === OK
      ...
      " schema is private" in {
        val (status, res) = schema.getMetadata(vendors, names, formats,
          versions, faultyVendorPrefix, permission)
        status === Unauthorized
        res must contain("You do not have sufficient privileges")
      }
    ...
    "for getPublicSchemas" should {
      ...
      "retrieve every public schema available" in {
        val (status, res) = schema.getPublicSchemas(vendorPrefix, permission)
        status === OK
        res must contain(innerSchema)
    ...
    "for getPublicMetadata" should {
      ...
      "retrieve metadata about every public schema available" in {
        val (status, res) = schema.getPublicMetadata(vendorPrefix, permission)
        status === OK
    ...
    "for getMetadataFromFormat" should {
      ...
      "retrieve schemas properly if they are public" in {
        val (status, res) = schema.getMetadataFromFormat(otherVendors,
          name2s, formats, faultyVendorPrefix, permission)
        status === OK
      ...
      " schema is private" in {
        val (status, res) = schema.getMetadataFromFormat(vendors, names,
          formats, faultyVendorPrefix, permission)
        status === Unauthorized
        res must contain("You do not have sufficient privileges")
      }
    ...
    "for getFromName" should {
      ...
      "retrieve schemas properly if they are private" in {
        val (status, res) =
          schema.getFromName(vendors, names, vendorPrefix, permission)
        status === OK
        res must contain(innerSchema2)
      ...
      "retrieve schemas properly if they are public" in {
        val (status, res) = schema.getFromName(otherVendors, name2s,
          faultyVendorPrefix, permission)
        status === OK
        res must contain(innerSchema)
      ...
      " schema is private" in {
        val (status, res) =
          schema.getFromName(vendors, names, faultyVendorPrefix, permission)
        status === Unauthorized
        res must contain("You do not have sufficient privileges")
      }
      ...
      "return a 404 if there are no schemas matching the query" in {
        val (status, res) =
          schema.getFromName(vendors, faultyNames, vendorPrefix, permission)
        status === NotFound
    ...
    "for getMetadataFromName" should {
      ...
      "retrieve schemas properly if they are private" in {
        val (status, res) =
          schema.getMetadataFromName(vendors, names, vendorPrefix, permission)
        status === OK
    ...
    "for getFromVendor" should {
      ...
      "return schemas properly if they are private" in {
        val (status, res) =
          schema.getFromVendor(vendors, vendorPrefix, permission)
        status === OK
        res must contain(innerSchema)
      ...
      "retrieve schemas properly if they are public" in {
        val (status, res) =
          schema.getFromVendor(otherVendors, faultyVendorPrefix, permission)
        status === OK
        res must contain(innerSchema)
      ...
      " schema is private" in {
        val (status, res) =
          schema.getFromVendor(vendors, faultyVendorPrefix, permission)
        status === Unauthorized
        res must contain("You do not have sufficient privileges")
      }
      ...
      "return a 404 if there are no schemas matching the query" in {
        val (status, res) =
          schema.getFromVendor(faultyVendors, vendorPrefix, permission)
        status === NotFound
    ...
    "for getMetadataFromVendor" should {
      ...
      "return schemas properly if they are private" in {
        val (status, res) =
          schema.getMetadataFromVendor(vendors, vendorPrefix, permission)
        status === OK
      ...
      "retrieve schemas properly if they are public" in {
        val (status, res) = schema.getMetadataFromVendor(otherVendors,
          faultyVendorPrefix, permission)
        status === OK
      ...
      " schema is private" in {
        val (status, res) =
          schema.getMetadataFromVendor(vendors, faultyVendorPrefix, permission)
        status === Unauthorized
        res must contain("You do not have sufficient privileges")
      }
      ...
      "return a 404 if there are no schemas matching the query" in {
        val (status, res) =
          schema.getMetadataFromVendor(faultyVendors, vendorPrefix, permission)
        status === NotFound
        res must contain("There are no schemas for this vendor")
    ...
    "for validate" should {
      ...
      "return a 200 if the instance is valid against the schema" in {
        ...
        schema.add(vendor, name, format, version,
          ...
          }""", vendorPrefix, permission, isPublic)
        val (status, res) =
